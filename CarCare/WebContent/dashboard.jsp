...
<body class="bg-gray-800">
	<div class="flex w-full">
        <div class="w-1/6 bg-gray-800 text-white p-8 rounded-r-lg shadow-md min-h-screen">
            <h2 class="text-2xl mb-4">${userRole} 님</h2>
            <ul>
                <li><a href="adminSelect.car" class="hover:text-white">[1]예약조회 및 관리</a></li>
                <li><a href="adminProductSelect.car" class="hover:text-white">[2]등록된 자동차 조회</a></li>
                <li><a href="adminPostProduct.car" class="hover:text-white">[3]자동차 상품 등록</a></li>
            </ul>
        </div>
<div class="w-5/6 bg-gray-200 p-8 shadow-md">
    <div class="text-4xl mb-8">${userRole}님의 관리자페이지</div>
    <div class="bg-gray-300 p-8 mb-8 shadow-inner rounded-md">영역구분/유동적으로 메뉴 불러오는 부분</div>

<c:if test="${not empty list}">
    <div class="bg-gray-100 shadow-md p-8 rounded-md">
        <table class="w-full rounded-md text-sm">
            <thead>
                <tr class="bg-gray-800 text-white">	
                    <th class="p-3">예약번호</th>
                    <th class="p-3">날짜</th>
                    <th class="p-3">모델</th>
                    <th class="p-3">신청자이름</th>
                    <th class="p-3">CC</th>
                    <th class="p-3">색상</th>
                    <th class="p-3">등급</th>
                    <th class="p-3">KM</th>
                    <th class="p-3">가격</th>
                    <th class="p-3">예약상태</th>
                    <th class="p-3">관리</th>
                </tr>
            </thead>
...
<c:if test="${not empty ModifyList}">
    <div class="modal" id="editModal">
        <div class="modal-content bg-white mx-auto mt-20 p-8 w-96 shadow-lg rounded-lg">
            <form id="editForm" action="adminUpdate.car" method="post" class="space-y-4">
                <label for="editId" class="block text-sm font-bold">예약번호:</label>
                <input type="text" id="editId" name="id" value="${ModifyList.id}" readonly
                    class="w-full border rounded p-2" />

                <label for="editDate" class="block text-sm font-bold">예약날짜:</label>
                <input type="text" id="editDate" name="date" value="${ModifyList.date}"
                    class="w-full border rounded p-2" />
                ...
                <input type="text" id="editModel" name="model" value="${ModifyList.model}" readonly
                    class="w-full border rounded p-2" />

                <label for="editName" class="block text-sm font-bold">이름:</label>
                <input type="text" id="editName" name="name" value="${ModifyList.name}" readonly
                    class="w-full border rounded p-2" />
...
<c:if test="${not empty brandList}">
    <div class="container mx-auto p-8 bg-gray-100 rounded-lg shadow-md">
    ...
    <div class="container mx-auto p-8 bg-gray-100 rounded-lg shadow-md mt-8">
    <form action="adminModelWrite.car" method="post" class="text-center">
        <h2 class="text-3xl mb-4 font-semibold">모델 등록</h2>
        
        <label for="brand" class="block mb-2 text-sm font-medium text-gray-600">브랜드:</label>
